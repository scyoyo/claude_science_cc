...
import {
  Users, Bot, MessageSquare, Code, Wand2, Plus,
  Settings, Workflow,
} from "lucide-react";
      ...
      {error || !stats ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard label={td("teams")} value={stats.total_teams} icon={Users} />
            <StatCard label={td("agents")} value={stats.total_agents} icon={Bot} />
            <StatCard label={td("meetings")} value={stats.total_meetings} icon={MessageSquare} />
            <StatCard label={td("artifacts")} value={stats.total_artifacts} icon={Code} />
          </div>
                    ...
                    {stats.teams_overview.map((team) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 shrink-0 ml-3 text-xs text-muted-foreground">
                          <span>{team.agent_count} {td("agents").toLowerCase()}</span>
                          <span>{team.meeting_count} {td("meetings").toLowerCase()}</span>
                        </div>
                      </Link>
                    ))}
                  </div>
